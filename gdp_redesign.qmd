...
- **Design Rationale:**
  ...
  - **Unified Region Colors:** A colour-blind-safe palette (Okabe & Ito, 2008) links regions across all bar and line charts.
...
# Color Palette (Okabe & Ito, 2008, colour-blind-safe)
region_colors <- c(
  Asia     = "#D55E00",  # vermillion
...
**Key insight:** Since 2000, Asia has overtaken the Americas as the world's largest regional economy: its share of world GDP climbed from 29% to 35%, while the Americas fell from 39% to 34% and Europe fell from 28% to 26%.
...
plot_c_trends <- ggplot(
  ...
  geom_text(
  data = label_data,
  aes(label = sprintf("%.0f%%", share * 100), vjust = vjust_val),
  fontface = "bold",
  size = 3.2,
  show.legend = FALSE
) +
