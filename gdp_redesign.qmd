...
- Population: **World Bank `SP.POP.TOTL`**, downloaded in-code via `WDI` and merged to derive GDP per capita.
...
- **Core Insight:** A higher nominal GDP does not automatically correlate to a higher GDP per capita. Massive market scale (e.g., China or India) often masks lower individual affluence due to population size, while smaller total economies (e.g., Switzerland or Australia) frequently lead in individual prosperity.
- **Design Rationale:**
  ...
  - **Unified Region Colors:** A cohesive color-blind-friendly palette links regions across all bar, and line charts.
...
The IMF file is GDP only, so population is joined from the **World Bank** indicator
**`SP.POP.TOTL`** (via `WDI`, which downloads it in-code) to derive GDP per capita —
separating economic *size* from *prosperity*. *(This chunk needs internet at render time;
run it once and it writes the result for offline re-renders.)*
...
pop <- WDI(
  ...
  filter(!is.na(iso3)) |>
  group_by(iso3) |>
  slice_max(year, n = 1, with_ties = FALSE) |> # latest available population
  ungroup() |>
